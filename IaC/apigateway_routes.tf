...
resource "aws_apigatewayv2_route" "create_pedido" {
  ...
  authorizer_id = aws_apigatewayv2_authorizer.users-authorizer
  target    = "integrations/${aws_apigatewayv2_integration.dotlanches_api.id}"
}
...
resource "aws_apigatewayv2_route" "create_produto" {
  ...
  authorization_type = "JWT"
  authorizer_id = aws_apigatewayv2_authorizer.management-authorizer
  target    = "integrations/${aws_apigatewayv2_integration.dotlanches_api.id}"
}
